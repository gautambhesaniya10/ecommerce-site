...
const Create = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const imgLink = URL.createObjectURL(media);
    console.log("imgLinkimgLink----", imgLink);
  ...
  const imageUpload = async () => {
    ...
    data.append("cloud_name", "dtxdo2h9m");
    const res = await fetch("https://api.cloudinary.com/v1_1/dtxdo2h9m/image/upload", {
      method: "POST",
      body: data,
    });
    const res2 = await res.json();
